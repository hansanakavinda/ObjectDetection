...
out_video = None  # Video writer for saving webcam feed

# Directories for saving images and videos
image_dir = "saved_images"
video_dir = "saved_live_feed"
...
def detect_objects():
    ...
    save_image_with_objects(img, file_path)
...
def start_live_detection():
    global video_capture, video_streaming, video_thread, mode, out_video
    ...
    # Set up video writer for saving live feed
    timestamp = time.strftime("%Y%m%d_%H%M%S")  # Format: YYYYMMDD_HHMMSS
    video_path = os.path.join(video_dir, f"live_feed_{timestamp}.mp4")
...
def stop_live_detection():
    global video_capture, video_streaming, mode, out_video
    video_streaming = False
...
def update_live_frame():
    global video_capture, video_streaming, out_video
    ...
    while video_streaming and video_capture.isOpened():
        ...
        if out_video:
            out_video.write(img_with_boxes)
...
# Function to save image with detected objects
def save_image_with_objects(img, original_file_path):
    image_name = os.path.basename(original_file_path)
    save_path = os.path.join(image_dir, f"detected_{image_name}")
    img.save(save_path)
